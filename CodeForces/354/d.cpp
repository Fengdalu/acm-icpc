...
int dfs(int x, int y, int d) {
    ...
    if(g[x][y][0][d]) cmin(dp[x][y][d], dfs(x - 1, y, d) + 1);
    if(g[x][y][1][d]) cmin(dp[x][y][d], dfs(x, y + 1, d) + 1);
    if(g[x][y][2][d]) cmin(dp[x][y][d], dfs(x + 1, y, d) + 1);
    if(g[x][y][3][d]) cmin(dp[x][y][d], dfs(x, y - 1, d) + 1);
    return dp[x][y][d];
}

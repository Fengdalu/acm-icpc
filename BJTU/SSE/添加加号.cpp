...
BigNum cal(std::string &s, int l, int r, int m) {
    vis[l][r][m] = true;
    if(m == 0) {
        return dp[l][r][m] = BigNum(s.substr(l, r - l + 1));
    }
    dp[l][r][m] = BigNum(s.substr(l, r - l + 1));
    for(int i = l; i < r; i++) {
        for(int j = 0; j <= m - 1; j++) {
            if(j > i - l || m - 1 - j > r - i - 1) continue;
            BigNum t = cal(s, l, i, j) + cal(s, i + 1, r, m - 1 - j);
            if(t < dp[l][r][m]) {
                dp[l][r][m] = t;
            }
        }
